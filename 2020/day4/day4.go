...
var passRE = regexp.MustCompile(`^([a-z]{3}):(.+?)$`)
var heightRE = regexp.MustCompile(`^(\d+)(cm|in)$`)
var hclRE = regexp.MustCompile(`^#[\da-f]{6}$`)
...
func main() {
	fmt.Printf("Part1: %v\n", CountMatch(passports, ValidPart1))
	fmt.Printf("Part1: %v\n", CountMatch(passports, ValidPart2))
}

func CountMatch(ps []Passport, f func(p Passport) bool) (t int) {
	for _, p := range ps {
		if f(p) {
			t++
		}
...
func ValidPart1(p Passport) bool {
	return p.byr != "" && p.iyr != "" && p.eyr != "" && p.hgt != "" && p.hcl != "" && p.ecl != "" && p.pid != ""
}

func ValidPart2(p Passport) bool {
	...
	hgtm := heightRE.FindStringSubmatch(p.hgt)
	if len(hgtm) == 0 {
		return false
	}
	switch hgtm[2] {
	case "cm":
		if hgtm[1] < "150" || hgtm[1] > "193" {
			return false
		}
	case "in":
		if hgtm[1] < "59" || hgtm[1] > "76" {
			return false
		}
	default:
		return false
...
func ParsePassport(data string) (p Passport) {
	for _, part := range strings.Split(data, " ") {
		match := passRE.FindStringSubmatch(part)
		if len(match) == 0 {
			continue
		}
...
func init() {
	...
	for _, row := range strings.Split(input, "\n") {
		if row == "" {
			ps = append(ps, ParsePassport(strings.Join(rs, " ")))
			rs = []string{}
		} else {
			rs = append(rs, row)
		}
