...
var containsre = regexp.MustCompile(`^(.+?) bags? contains? (.+?)$`)
var bags = regexp.MustCompile(`^(\d+) (.+?) bags?\.?$`)

type rule map[string]int

var rules map[string]rule
var containedBy map[string]rule

func main() {
	fmt.Printf("Part 1: %v\n", part1(containedBy, "shiny gold"))
	fmt.Printf("Part 2: %v\n", part2(rules, "shiny gold"))
}

func part1(c map[string]rule, bag string) (t int) {
	visited := map[string]bool{}
	return len(visit(c, bag, visited))
}

func part2(c map[string]rule, bag string) (t int) {
	for k, v := range c[bag] {
		t += v * (1 + part2(c, k))
	}
	return
...
func visit(c map[string]rule, bag string, visited map[string]bool) (m map[string]bool) {
...
func init() {
	...
	for _, l := range strings.Split(input, "\n") {
		ruleMatch := containsre.FindStringSubmatch(l)
		contains := strings.Split(ruleMatch[2], ", ")
		r := make(rule)
		rules[ruleMatch[1]] = r
		for _, c := range contains {
			bagMatch := bags.FindStringSubmatch(c)
			if len(bagMatch) > 0 {
				r[bagMatch[2]], _ = strconv.Atoi(bagMatch[1])
			}
		}
	}

	containedBy = make(map[string]rule)
	for k, r := range rules {
		for bk, _ := range r {
			if _, ok := containedBy[bk]; !ok {
				containedBy[bk] = make(rule)
			}
			containedBy[bk][k] = 1
		}
	}
...
var input = `muted tomato bags contain 1 bright brown bag, 1 dotted gold bag, 2 faded gray bags, 1 posh yellow bag.
